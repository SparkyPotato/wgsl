...
use crate::{
	ast::*,
	diagnostic::{Diagnostic, Diagnostics, Span},
...
pub fn parse(source: &str, intern: &mut Interner, diagnostics: &mut Diagnostics) -> TranslationUnit {
...
impl Parser<'_> {
	fn parse(mut self, source: &str) -> (Option<TranslationUnit>, Vec<Simple<TokenKind, Span>>) {
		fn get_text(source: &str, span: Span) -> &str { &source[span.start as usize..span.end as usize] }
		...
		let text = just(TokenKind::Word).map_with_span(|_, span: Span| intern.borrow_mut().get(get_text(source, span)));
		...
		let stmt = recursive(|stmt| {
			...
			let case_selector = kw("default").to(CaseSelector::Default).or(kw("case").ignore_then(
				kw("default")
			...
			let case = case_selector
				...
				.then(block.clone())
				.map_with_span(|(selectors, block), span| Case { selectors, block, span })
				.boxed();
			...
			let with_semi = choice((
				...
				kw("return").ignore_then(expr.clone().or_not()).map(StmtKind::Return),
			...
			choice((
				...
				kw("continuing").ignore_then(block.clone()).map(StmtKind::Continuing),
		...
		let lit = choice((
			...
			just(TokenKind::IntLit)
				.map_with_span(|_, span| parse_int(get_text(source, span), span, *diagnostics.borrow_mut())),
			just(TokenKind::FloatLit)
				.map_with_span(|_, span| parse_float(get_text(source, span), span, *diagnostics.borrow_mut())),
		));
			...
			.foldl(|f, access| {
				...
				Expr {
					kind: match access.0 {
						...
						CallIndexAccess::Index(index) => ExprKind::Index(Box::new(f), Box::new(index)),
						CallIndexAccess::Access(field) => ExprKind::Member(Box::new(f), field),
						CallIndexAccess::Postfix(op) => ExprKind::Postfix(PostfixExpr { expr: Box::new(f), op }),
					},
					span,
				}
			})
		...
		let over = attribs
			...
			.then(just(TokenKind::Equal).ignore_then(expr.clone()).or_not())
			.then_ignore(just(TokenKind::Semicolon))
			.map(|(((attribs, name), ty), val)| Override { attribs, name, ty, val })
			.boxed();
		...
		tu.parse_recovery(Stream::from_iter(
			self.lexer.eof_span(),
			std::iter::from_fn(|| self.lexer.next().map(|Token { kind, span }| (kind, span))),
		))
	}
}

fn parse_int(text: &str, span: Span, diagnostics: &mut Diagnostics) -> Literal {
	...
	match value {
		Ok(value) => {
			if signedness == b'i' {
				...
				match i32 {
					...
					Err(_) => {
						diagnostics.push(span.error("integer literal is too large for i32"));
						Literal::I32(0)
					},
				}
				...
				match u32 {
					...
					Err(_) => {
						diagnostics.push(span.error("integer literal is too large for u32"));
						Literal::U32(0)
					},
				}
			} else {
		...
		Err(_) => {
			diagnostics.push(span.error("integer literal too large") + span.marker());
			Literal::AbstractInt(0)
		},
	}
}

fn parse_float(text: &str, span: Span, diagnostics: &mut Diagnostics) -> Literal {
	...
	match value {
		...
		None => {
			diagnostics.push(span.error("float literal could not be parsed") + span.marker());
			Literal::AbstractFloat(0.0)
		},
	}
}

fn error_to_diagnostic(error: Simple<TokenKind, Span>) -> Diagnostic {
	let span = error.span();
	...
	match error.reason() {
		SimpleReason::Unexpected => {
			...
			match error.found() {
				Some(tok) => {
					span.error(format!("unexpected `{}`", tok))
						+ span.label({
				...
				None => {
					span.error("unexpected end of file")
						+ span.label({
							let mut s = "expected one of ".to_string();
							comma_sep(&mut s, expected);
							s
		...
		SimpleReason::Unclosed { span, delimiter } => span.error(format!("unclosed `{}`", delimiter)) + span.marker(),
		SimpleReason::Custom(message) => span.error(message) + span.marker(),
	}
}
